...
static double force_halo_z (const float& R, const float& z){
    double r = std::sqrt(R*R + z*z);
    return  G*Mh/r*(1/d/(1+r/d) - std::log(1+r/d)/r)*z/r;
};


//Tmax, z, X, Y, vz, vt, vr, dt, frequency, z_ij in kpc
std::vector<std::vector<std::vector<double>>> RK4_shift(double Tmax, std::vector<double> Z, 
     ...
     std::vector<float> z_ij)
{
    ...
    auto force_R = [](const std::vector<float>& coord, std::vector<float>& z){
        double R = std::sqrt(coord[0]*coord[0]+coord[1]*coord[1]);
        return force_buldge_R(R, coord[2])+force_disk_R(coord, z)+force_halo_R(R, coord[2]); //
    };
    auto force_z = [](const std::vector<float>& coord, std::vector<float>& z){
        double R = std::sqrt(coord[0]*coord[0]+coord[1]*coord[1]);
        return force_buldge_z(R, coord[2])+force_disk_z(coord, z)+force_halo_z(R, coord[2]);
    };
    ...
    for (int i = 0; i<static_cast<int>(z_ij.size()); i++){
        myfile << z_ij[i] << ",";
    }
    myfile << "\n";
    myfile.close();
...
PYBIND11_MODULE(simple_projection, m) {
    m.def("RK4_shift", &RK4_shift, "Tmax, z, X, Y, vz, vt, vr, dt, frequency, z_ij, d, r");
